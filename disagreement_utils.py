...
def get_pairwise_disagreement(key_a, key_b, datasets):
    ...
    return np.array([[X[i] / np.max(X), y[i]*100] for i in range(len(X))[:len(X) - 1]])


def non_cum_percentage_disagreement(key_a, key_b, datasets, n, m):
    pairs = sample_pairs(datasets, n)
    data = []
    for p in pairs:
        summary_i, summary_j = p
    ...
    y = []
    for i in range(m):
        bin_ = [d for d in data if (d[0] >= min_score + i * bin_size) and (d[0] <= min_score + (i+1) * bin_size)]
        if len(bin_) > 500:
            perc = sum([-d[1] for d in bin_ if d[1] < 0]) / float(len(bin_))
            X.append(i * bin_size)
            y.append(perc)

    print(key_a, key_b, len(X))
    return X, y


def non_cum_get_pairwise_disagreement(key_a, key_b, datasets):
    X, y = non_cum_percentage_disagreement(key_a, key_b, datasets, 100000, 15)
    return np.array([[X[i] / np.max(X), y[i]*100] for i in range(len(X))[:len(X) - 1]])


def proportion_better(datasets, metrics=['bert_recall_score', 'mover_score', 'rouge_1_recall', 'rouge_2_recall', 'rouge_l_recall'], m=1, min_avg=0):
    datapoints = []
    for _ in range(m):
        dataset = random.choice(datasets)
        topic_key = random.choice(list(dataset.keys()))
...
def proportion_worse(datasets, metrics=['bert_recall_score', 'mover_score', 'rouge_1_recall', 'rouge_2_recall', 'rouge_l_recall'], m=1, min_avg=0):
    datapoints = []
    for _ in range(m):
        dataset = random.choice(datasets)
        topic_key = random.choice(list(dataset.keys()))
