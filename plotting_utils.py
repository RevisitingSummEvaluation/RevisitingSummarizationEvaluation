...
import itertools
import numpy as np
from disagreement_utils import pairwise_correlation, top_matrix_example, pairwise_correlation_top_avg
...
def plot_proportion_improvements(datapoints, show_ylabel=True):
    ...
    if show_ylabel:
        ax.set_ylabel(r'$\frac{F}{N}$', fontsize=45)
    else:
        ax.set_ylabel('')
    ax.tick_params(axis='both', labelsize=25)


def plot_proportion_worse(datapoints, show_ylabel=True):
    x = [t for t, v in datapoints]
    y = [v for t, v in datapoints]
